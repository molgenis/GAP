...
with open(args.output_file, "w") as output_handle:
    ...
    output_handle.write(delim.join(["SNP Name", "Sample ID", "Chr", "MapInfo" ,"Snp" , "GType" ,"Allele1 - Top","Allele2 - Top","X_raw","Y_raw","X","Y"]) + "\n")
	...
	base_calls = GenotypeCalls( gtc_file ).get_base_calls()
        ...
        for (name, chrom, map_info, snp, genotype, norm, Allele, raw_x, raw_y) in zip(manifest.names, manifest.chroms, manifest.map_infos, manifest.snps ,genotypes , normalized_intensities, base_calls, raw_Xs, raw_Ys):

            x_norm = norm[0]
            y_norm = norm[1]

            output_handle.write(delim.join([name, os.path.basename(gtc_file)[:-4], str(chrom) , str(map_info) , str(snp), code2genotype[genotype] , Allele[:-1] , Allele[1:] , str(raw_x), str(raw_y) ,str(x_norm), str(y_norm)]) + "\n")
