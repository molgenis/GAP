...
#opt$input<-"/groups/umcg-aad/tmp04/umcg-elopera/allb_autosome_QC/6_PCA"
...
eur.samples<-G_eigenvec[G_eigenvec$sampletype=="eur" & G_eigenvec$Population!="FIN",c(1,2)]
...
##aesthetics
ELcolor<-c("chartreuse3",  "plum3", "brown", "orange" , "blue","cyan1", "darkgray")
...
if (opt$res=="2nd.") {pops.plot<- ggplot(G_eigenvec,aes(x=G_eigenvec$`PC 1`, y=G_eigenvec$`PC 2`))+
  ...
  theme_bw()+
  scale_colour_manual(values=ELcolor[c(1,3,2,4,6,5,7)])+
...
pops.plot<- ggplot(G_eigenvec,aes(x=G_eigenvec$`PC 1`, y=G_eigenvec$`PC 2`))+
  geom_point(aes(colour=Population, shape=Population),alpha=0.4,size=2)+
  theme_bw()+
  scale_colour_manual(values=c(viridis(8),"brown",rainbow(19))) +
...
if (opt$res=="2nd.") superpops.plot<-"No Superpopulations" else {
  ...
  superpops.plot<-ggplot(G_eigenvec,aes(x=G_eigenvec$`PC 1`, y=G_eigenvec$`PC 2`))+
    geom_point(aes(colour=superpop, shape=superpop),alpha=0.4, size=2)+
    theme_bw()+
